    ...
    <div className="max-w-6xl mx-auto space-y-20 animate-in fade-in slide-in-from-bottom-8 duration-1000 pb-32">
      <div className="text-center space-y-6">
        <motion.div 
          ...
          className="inline-flex items-center space-x-3 px-6 py-2 bg-emerald-50 rounded-full border border-emerald-100 mb-4"
        >
          <Cpu className="w-4 h-4 text-emerald-600" />
          <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Protocol v2.1.0</span>
        </motion.div>
        <h1 className="text-6xl lg:text-[100px] font-black text-gray-900 tracking-tighter leading-[0.85]">Developer <br /><span className="text-emerald-600">Gateway.</span></h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         <div className="p-10 bg-gray-950 text-white rounded-[3rem] shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center mb-6 text-emerald-500">
         ...
         <div className="p-10 bg-emerald-600 text-white rounded-[3rem] shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
      ...
      <div className="p-16 bg-emerald-600 rounded-[4rem] text-white text-center space-y-8 shadow-2xl shadow-emerald-200">
         ...
         <div className="pt-4">
            <button className="px-12 py-6 bg-white text-emerald-600 rounded-3xl font-black text-[11px] uppercase tracking-widest hover:bg-gray-900 hover:text-white transition-all transform active:scale-95 shadow-xl">Go to Dashboard</button>
         </div>
      </div>
    </div>
  );
...
const DocSection = ({ title, icon, children }: any) => (
  <div className="bg-white p-12 lg:p-20 rounded-[4rem] border border-gray-100 shadow-sm relative overflow-hidden group">
     <div className="flex items-center space-x-6 mb-10">
        <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-all">
          {icon}
        </div>
     ...
     <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-50/30 rounded-full translate-x-1/2 -translate-y-1/2 blur-3xl"></div>
  </div>
);
...
const CodeBlock = ({ title, code }: { title: string, code: string }) => (
  <div className="space-y-3">
    ...
    <div className="bg-gray-950 p-8 rounded-b-2xl font-mono text-sm text-emerald-400 border border-gray-900 overflow-x-auto whitespace-pre shadow-inner">
       {code}
    </div>
  </div>
);
