...
const DashboardPage: React.FC = () => {
  ...
  const handlePaystackUpgrade = () => {
    addNotification("Insufficient balance. Initializing upgrade funding...", "info");
    const handler = PaystackPop.setup({
      ...
      callback: (response: any) => {
        addNotification("Wallet funded! Upgrading account...", "success");
  ...
  return (
    <>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 space-y-10 pb-20">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row justify-between lg:items-end gap-12">
            <div className="space-y-6">
              <div className="flex items-center space-x-3">
                 <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${user?.role === 'admin' ? 'bg-red-500 shadow-lg shadow-red-500/20' : user?.role === 'reseller' ? 'bg-green-600 shadow-lg shadow-green-500/20' : 'bg-blue-600 shadow-lg shadow-blue-500/20'}`}>
                    {user?.role?.toUpperCase()} NODE
                 </div>
                 <div className="text-white/40 font-black text-[9px] uppercase tracking-widest bg-white/5 px-4 py-1.5 rounded-full border border-white/10">@{user?.username}</div>
              </div>
              <div>
                 <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30 mb-2">Operational Liquidity</p>
                 <div className="text-5xl lg:text-7xl font-black tracking-tighter leading-none flex items-baseline">
                    <span className="text-2xl mr-1 text-white/40">₦</span>
              ...
              <div className="flex flex-wrap items-center gap-4">
                 <div className="bg-white/5 px-5 py-3 rounded-2xl border border-white/10">
                    <p className="text-[7px] font-black text-white/20 uppercase tracking-widest mb-1">Referral Key</p>
                    <p className="font-black text-blue-400 tracking-tight text-sm uppercase">{user?.referralCode || 'N/A'}</p>
                 </div>
                 {user?.role === 'user' && (
                   <button 
                    onClick={() => handleResellerUpgrade()}
                    disabled={isUpgrading}
                    className="bg-white text-gray-900 px-6 py-3.5 rounded-2xl font-black text-[9px] uppercase tracking-widest transition-all hover:bg-blue-600 hover:text-white shadow-xl flex items-center gap-2 active:scale-95 disabled:opacity-50"
                   >
                      {isUpgrading ? <Spinner /> : <><ShieldCheckIcon /> Upgrade (₦1,200)</>}
                   </button>
                 )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full lg:w-auto">
               ...
               <ActionBtn onClick={() => navigate('/bills')} icon={<BoltIcon />} label="Power" />
        ...
        {/* Feature Carousel */}
        <div className="relative h-64 lg:h-80 rounded-[3rem] overflow-hidden group shadow-2xl border-4 border-white animate-in zoom-in-95 duration-700">
           {slides.map((slide, idx) => (
             <div key={idx} className={`absolute inset-0 transition-opacity duration-1000 ${idx === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-12 flex flex-col justify-end">
                   <h3 className="text-2xl lg:text-4xl font-black text-white tracking-tighter leading-none mb-3">{slide.title}</h3>
                   <p className="text-white/60 text-sm lg:text-lg font-medium max-w-lg">{slide.desc}</p>
                </div>
             </div>
           ))}
        </div>
        ...
        {announcement && (
          <div className="bg-blue-600 p-6 rounded-3xl flex items-center space-x-4 shadow-xl shadow-blue-200 animate-pulse">
        ...
        {/* History Section */}
        <div className="bg-white rounded-[3rem] p-10 lg:p-14 shadow-2xl border border-gray-100">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-2xl font-black text-gray-900 tracking-tighter">Terminal History</h3>
              ...
              {recentTransactions.length > 0 ? recentTransactions.map((tx) => (
                ...
                >
                  <div className="flex items-center space-x-5">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black shadow-sm ${tx.status === 'SUCCESS' ? 'bg-blue-600 text-white' : 'bg-red-50 text-red-600'}`}>
                      {tx.type.charAt(0)}
                    </div>
                    <div>
                      <p className="font-black text-gray-900 text-lg tracking-tight">{tx.source}</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                        {tx.date_created?.seconds ? new Date(tx.date_created.seconds * 1000).toLocaleTimeString() : 'Processing Node...'}
                      </p>
                    </div>
                  </div>
                ...
                <div className="py-24 text-center">
                   <div className="w-20 h-20 bg-gray-50 rounded-[2rem] flex items-center justify-center mx-auto mb-6 text-gray-200 shadow-inner"><HistoryIcon /></div>
                   <h4 className="font-black text-gray-900 text-xl tracking-tighter">No Activity Logged</h4>
                   <p className="text-gray-400 text-sm mt-2 max-w-xs mx-auto">Your digital footprints will appear here once you initiate a service node.</p>
                </div>
              )}
            </div>
          )}
