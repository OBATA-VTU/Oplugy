...
const ComingSoonPage: React.FC<ComingSoonPageProps> = ({ title, description }) => {
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6 bg-white rounded-[3rem] border border-gray-50 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mb-8 animate-pulse">
      ...
      <Link 
        to="/dashboard" 
        className="bg-gray-900 text-white px-10 py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-600 transition-all transform hover:-translate-y-1"
      >
        Back to Dashboard
      </Link>
    </div>
